...
Forty-two primary raw JVMTI JSONL traces, JaCoCo execution data/class dumps, ASM raw maps, and Gradle output remain local under `/private/tmp/task19-primary-v2`. The compact `stp-spring-core-spike/task19/evidence/raw-trace-hashes.json` records every primary oracle trace's byte size and SHA-256. Four no-oracle controls remain under `/private/tmp/task19-no-oracle`, two tests' additional triple runs under `/private/tmp/task19-repeat`, and order-smoke material under `/private/tmp/task19-order-{jacoco-asm,asm-jacoco}`. They are reproducible with the TASK 19 scripts and excluded because joined observations and summaries retain the claims without duplicating raw execution streams or class dumps.
