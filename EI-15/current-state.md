...
The normal automatic PR API is `stpLookupCoverageMap` → `stpPrSelect` → `smartTestPicker`. GitHub Branch Source `SOURCE_HEAD` is the only demonstrated automatic provider path. The explicit `stpHeadInventory`/`stpExplicitPrSelect` path supports Maven. These low-level steps are not mandatory alternatives to the six normal workflow steps.
...
| Jenkins HPI | `io.github.smarttestpicker:smart-test-picker:1.0-SNAPSHOT`, Jenkins revision | Jenkins 2.516.2; tested JDK 21; bytecode 17 | all rows above; adapter/plan contract 1 | HPI contains generated manifest/checksums and `WEB-INF/licenses.xml` | 96 Jenkins tests and HPI build |
...
Unsupported boundaries include Maven+ASM, Gradle included/composite builds for executable mapping, arbitrary parallel test forks, automatic SCM providers other than GitHub Branch Source, automatic Maven inventory inside `stpPrSelect`, synthetic-merge selective inventory, multi-controller publication, remote backends other than RAW_HTTP, direct reactive/reflection context propagation, release signing/publishing, and universal JUnit/build-tool compatibility.

EI-15 completed clean initially-empty-cache builds for both repositories and the Java-21 packaged Docker smoke. The direct `smartTestPicker` selector mode has no authoritative inventory input and therefore conservatively runs the full suite; selective automatic operation uses the normal inventory-producing PR workflow, while explicit plans remain supported for both adapters. Release prerequisites still unmet are release signing/distribution and a source-repository LICENSE/NOTICE disposition for the Jenkins repository. A later production-behavior change must rerun its affected external Jenkins path. EI-15 does not publish artifacts.
