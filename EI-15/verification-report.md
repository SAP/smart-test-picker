...
Maintained entrypoint: `python3 verification/verify.py` in the Jenkins repository. It builds/inspects the HPI, launches the task-owned controller/agent, exercises packaged selector and Gradle/Maven adapters, reconciles exact reports for selected/NONE/RUN_ALL modes, tests checksum/contract failures before body execution, and covers cache corruption/upgrade/restart behavior. `STP_SOURCE_ROOT` makes the Docker source context explicit outside the historical sibling layout. Direct selection without an authoritative head inventory is asserted to produce safe `RUN_ALL`; the normal inventory-backed workflow is retained EI-13 evidence.
